...
int channel_add_client(Channel *channel, Client *client)
{
     ...
     */
    if(!(channel_broadcast_printf(channel, "@%s has joined #%s \n", client->nick, channel->name))){

        fprintf(stderr, "channel broadcast printf \n");
        return 0;
    }
...
int channel_remove_client(Channel *channel, Client *client)
{
    ...
    *client->channel_clients = NULL;
    ...
    if(channel_broadcast_printf(channel, "@%s has left  #%s \n", client->nick, channel->name) == 0){
